...
(function () {
  ...
  let saveBtn = document.getElementById("statusToggle-btn");
  saveBtn.addEventListener('click', function (e) {
    ...
    selectedRows.forEach(row => {
      ...
      selectedIndex.push(row.node.innerText.split('\t')[1]);
      //selectedIndex.push(row.getElementById("id").value)
      console.log(selectedIndex)
    });
    if (selectedIndex.length < 1) {
      alert("выберите пользователя для смены статуса")
      e.preventDefault();
    } else {

      //console.log(selectedIndex[0])
      document.getElementById("ids").value = selectedIndex[0];
      alert("статус пользователя сменен")
    }
  });
})();