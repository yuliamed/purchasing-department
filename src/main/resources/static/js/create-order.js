...
(function () {
    ...
    let saveBtn = document.getElementById("create-order-btn");
    saveBtn.addEventListener('click', function (e) {
        //e.preventDefault();
        let selectedIndex = []
        selectedRows.forEach(row => {
            selectedIndex.push(row.node.innerText.split('\t')[0]);

        });
        if (selectedIndex.length < 1) {
            alert("выберите поставщика для создания заказа")
            e.preventDefault();
        } else {
            let payId = document.getElementById("pay-type").value;
            let count = document.getElementById("amount").value;
            console.log(payId);

            //console.log(selectedIndex[0])
            document.getElementById("providerId").value = selectedIndex[0];
            document.getElementById("payment-type").value = payId.split(' ')[0];
