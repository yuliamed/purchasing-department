(function() {
  ...
  function toggleTableSelectedRow(tr, index) {
    if (tr && typeof index === 'number') {
      ...
      found = cells[4].getElementsByTagName("input")[0];
      if (found.hidden == true){found.hidden = false;}else{
        found.hidden = true;
      }
      if (tr.classList.contains('selected')) selectedRows.push({
  ...
  saveBtn.addEventListener('click', function (e) {
    ...
    selectedRows.forEach(row => {
      selectedPrices.push(row.node.getElementsByTagName("td")[3].getElementsByTagName("input")[0].value)
      selectedTime.push(row.node.getElementsByTagName("td")[4].getElementsByTagName("input")[0].value)
    });
    //console.log(selectedCounters)

    if (selectedIndex.length < 1) {
      alert("Для создания поставщика выберите хотя бы 1 ресурс")
      e.preventDefault();
    } else {
      document.getElementById("specifications").value = selectedIndex;
      document.getElementById("prices").value = selectedPrices;
